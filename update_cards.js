...
// The state injection from the previous failed python script
const stateInjection = `  const [isEditingSkills, setIsEditingSkills] = useState(false);
...
content = content.replace('  const [isEditingSkills, setIsEditingSkills] = useState(false);', stateInjection);
content = content.replace(/<SkillsEditor/g, '<TagEditor');
content = content.replace(/initialSkills=\{profile\.skills\}/g, 'initialTags={profile.skills}');
content = content.replace(/newSkills/g, 'newTags');

// Replace Target Roles Card
const targetRolesCard = `              {/* Target Roles Card */}
...
const locationsCard = `              {/* Locations & Work Mode Card */}
              <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl p-6">
                <div className="flex justify-between items-start mb-6">
                  <div>
...
const compensationCard = `              {/* Compensation Expectations */}
              <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl p-6">
                <div className="flex justify-between items-start mb-6">
                  <div>
...
const prioritiesCard = `              {/* Priorities & Deal Breakers */}
              <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl p-6">
                <div className="flex justify-between items-start mb-6">
                  <div>
...
content = content.replace(/\{\/\* Target Roles Card \*\/\}[\s\S]*?(?=\{\/\* Locations & Work Mode Card \*\/})/g, targetRolesCard + '\n\n              ');
content = content.replace(/\{\/\* Locations & Work Mode Card \*\/\}[\s\S]*?(?=\{\/\* Compensation Expectations \*\/})/g, locationsCard + '\n\n              ');
content = content.replace(/\{\/\* Compensation Expectations \*\/\}[\s\S]*?(?=\{\/\* Priorities & Deal Breakers \*\/})/g, compensationCard + '\n\n              ');
content = content.replace(/\{\/\* Priorities & Deal Breakers \*\/\}[\s\S]*?(?=\<\/div\>\n          \<\/div\>\n        \<\/div\>\n        \n        \{\/\* Footer \*\/})/g, prioritiesCard + '\n\n            ');
